-- Add Blueprint factor to compute_connector_score().
-- +15 if the member has published a Blueprint (one-time, existence-based).
-- A member can only have one Blueprint row (user_id is unique in blueprints),
-- so _blueprint is always 0 or 1 and the total contribution is 0 or 15.
-- Also bumps lend_a_hand multiplier from 2 to 10 (same migration for atomicity).
