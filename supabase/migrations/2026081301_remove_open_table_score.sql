-- 2026081301_remove_open_table_score.sql
-- Remove "completed Open Table room" as a Connector Score factor.
-- Open Table is discontinued; members can no longer earn new points from it.
-- The open_table_members / open_table_rooms tables are preserved untouched.
-- The Table Setter badge is NOT changed here — that requires a separate decision
-- on whether to repoint it to event attendance or park it entirely.
...
BEGIN
  ...
  -- Set-based query collapses N round-trips into one.
  WITH facilitated AS (
    SELECT
  ...
  -- +10 per can_help reaction followed by a message to the post author
  ...
  -- +15 if this member has published a Blueprint (one per member, 0 or 1 rows)
  SELECT COUNT(*) INTO _blueprint
  FROM blueprints
  WHERE user_id = _uid;
