...
// ── Invite request approval email ────────────────────────────────────────────

export function inviteApprovalEmail(
  toFullName: string,
  code: string,
  joinUrl: string,
): string {
  const firstName = escapeHtml(toFullName.split(' ')[0])
...
<body style="margin:0;padding:0;background:#F5F2EE;">
  <div style="font-family:${sans};max-width:480px;margin:0 auto;padding:48px 24px;background:#F5F2EE;">
    ...
    <div style="text-align:center;margin:0 0 28px;">
      <span style="display:inline-block;background:#C8F53C;color:#0F1B3C;font-family:'Courier New',Courier,monospace;font-size:22px;font-weight:700;letter-spacing:0.12em;padding:14px 32px;border-radius:100px;">${safeCode}</span>
    </div>
...
export function verificationSubmittedAdminEmail({
  memberName,
  memberEmail,
  statement,
}: {
  ...
  const statementHtml = `<p style="font-size:13px;color:#6B7280;font-weight:600;margin:0 0 8px;font-family:${sans};">Statement:</p>
    <blockquote style="border-left:3px solid #C8F53C;margin:0 0 28px;padding:12px 16px;background:#fff;border-radius:0 8px 8px 0;color:#0F1B3C;font-size:15px;font-style:italic;font-family:${sans};">${escapeHtml(statement)}</blockquote>`
...
<body style="margin:0;padding:0;background:#F5F2EE;">
  <div style="font-family:${sans};max-width:480px;margin:0 auto;padding:48px 24px;background:#F5F2EE;">
    ...
    <h1 style="font-size:26px;color:#0F1B3C;margin:0 0 16px;font-weight:900;line-height:1.15;font-family:${serif};">${escapeHtml(memberName)} wants to get verified.</h1>

    <table style="border-collapse:collapse;margin:0 0 20px;">
      <tr>
        <td style="font-size:13px;color:#6B7280;padding:3px 16px 3px 0;white-space:nowrap;vertical-align:top;font-family:${sans};">Email</td>
        <td style="font-size:13px;color:#0F1B3C;padding:3px 0;font-family:${sans};"><a href="mailto:${escapeHtml(memberEmail)}" style="color:#0F1B3C;text-decoration:none;">${escapeHtml(memberEmail)}</a></td>
      </tr>
    </table>
