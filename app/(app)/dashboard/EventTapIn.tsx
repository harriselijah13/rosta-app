...
export default function EventTapIn({ isTappedIn: initialTappedIn }: Props) {
  ...
  return (
    <div className="bg-white border border-border rounded-2xl px-5 py-5">
      {tappedIn ? (
        <>
          <p className="text-base font-semibold text-navy">You&apos;re tapped in.</p>
        ...
        <>
          <p className="text-sm font-medium text-navy mb-1">At a networking event today?</p>
          <p className="text-xs mb-4" style={{ color: 'rgba(107,114,128,0.65)' }}>
            Tap in to remember who you met. We&apos;ll help you turn the right conversations into connections.
          </p>
          <button
            onClick={handleTapIn}
            disabled={loading}
            className="text-xs font-medium text-navy border border-navy px-3.5 py-1.5 rounded-full hover:bg-navy hover:text-warm-white transition-colors disabled:opacity-50"
          >
            {loading ? '…' : 'Tap in'}
          </button>
        </>
